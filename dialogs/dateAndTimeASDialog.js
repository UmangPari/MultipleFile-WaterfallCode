...
class DateAndTimeASDialog extends ComponentDialog {
    ...
    async startDateStep(step)
      {
          step.context.sendActivity('check');
        return await step.prompt(TEXT_PROMPT,'enter date in yyyy-mm-dd format');
      }
      async startTimeStep(step)
      {
          asStartDate=step.result;
          return await step.prompt(TEXT_PROMPT,"enter time in hh:mm:ss");
      } 
